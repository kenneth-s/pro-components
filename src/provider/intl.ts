...
/**
 * `en-US` 的 messages 快照，供 `createIntl` 的跨语言 fallback 使用。
 *
 * 这里要显式保存一份 messages 引用，而不是在运行时从 `intlMap['en-US'].getMessage` 取值，
 * 避免"createIntl 自身被改动为其他 fallback 策略时出现递归"的风险。
 */
const enUSMessages: Record<string, any> = enUS;
 ...
 * 1. en-US locale 自身找不到 → 返回 `defaultMessage`
 * 2. 其他 locale 找不到 → 去 en-US messages 里找；还找不到才返回 `defaultMessage`
 *
...
export const createIntl = (
  ...
  getMessage: (id: string, defaultMessage: string) => {
    ...
    if (localeKeyDashed === 'en-US') {
      return defaultMessage;
    }
    // 使用本模块顶层缓存的 en-US messages，而不是反向查 intlMap['en-US']，
    // 避免 createIntl 实现演进时引入潜在递归。
    const fallback = getMessageByPath(enUSMessages, id);
